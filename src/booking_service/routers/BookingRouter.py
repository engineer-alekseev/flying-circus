...
from database.Models.Models import Role, User, Room, Booking
from database.database import get_session, AsyncSession, selectinload
...
async def get_room_list(
    ...
    if  not min_time <= delta <= max_time:  
        raise HTTPException(
            ...
            detail="Incorrect booking time. Min time: {} Max time: {} Requested time: {}".format(room.min_time, room.max_time, delta)
        )
        
    overlaps = get_overlap_bookings(room.id, booking.start_time, booking.end_time, session)
